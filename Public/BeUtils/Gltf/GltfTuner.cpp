...
public:
	...
	{
		// Need to lock to prevent other threads from trying to update the rules while we use them.
		// Need to remain locked for the whole Tune() process obviously so that the rules used remain consistant...
		BeUtils::RLock rlock(rulesMutex_);
	...
	Cesium3DTilesSelection::GltfModifierInput&& input)
{
	...
	std::optional<int64_t> currentVersion;
	{
		// IMPORTANT: this lock must remain short-lived because asyncSystem.runInWorkerThread might decide to run the
		// task immediately in the current thread, but Tune() will also try to acquire the same non-recursive lock
		BeUtils::RLock rlock(impl_->mutex_);
		currentVersion = getCurrentVersion();
	}
	if (currentVersion
		&& Cesium3DTilesSelection::GltfModifierVersionExtension::getVersion(input.previousModel) != (*currentVersion))
	{
		return input.asyncSystem.runInWorkerThread(
			// std::move compiles but does not move anything because the member is const!
			// And the copy _is_ needed here because the reference is on a member that is itself std::moved somewhere
			// else before this has a chance to run (see postProcessContentInWorkerThread, in the capture list of the
			// 'thenInWorkerThread' call chained after this 'apply')/
			// It's a shame because we obviously only need read access :/
			...
			-> std::optional<Cesium3DTilesSelection::GltfModifierOutput>
			{
				return Cesium3DTilesSelection::GltfModifierOutput{
					GltfTunerHelper(previousModel, impl_->mutex_, impl_->rulesEx_, impl_->materialHelper_,
								 ...
								 CesiumGltf::Model& tunedModel)
{
	...
	{
		tunedModel = std::move(
			GltfTunerHelper(model, impl_->mutex_, impl_->rulesEx_, impl_->materialHelper_, tileTransform_shifted)
				.Tune());
